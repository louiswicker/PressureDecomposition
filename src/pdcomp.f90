...
!               beta, pb, pdn, pdl,ptdn,fpb,fptdn,fpdn,          &
...
!               fpbz,fptdnx,fptdny,fptdnz)
  ...
  subroutine pdcomp(ni,nj,nk,wbc,ebc,sbc,nbc,dx,dy,          &
                    zh,rho0,th0,qv0,pi0,u0,v0,u,v,w,pavgin,  &
                    betaT, betaP, pb, pdn, pdl, ptdn, fpb, fptdn, fpdn,          &
                    pex,psh,psum,fpex,fpsh,fpsum,fpdl,fpdlx, &
                    fpdly,fpdlz,fpdnx,fpdny,fpdnz,fpbx,fpby, &
                    fpbz,fptdnx,fptdny,fptdnz)
...
! JT add
  ...
  real, intent(inout), dimension(ni,nj,nk) :: pb, pdn, pdl, betaT, betaP
...
!-----------------------------------------------------------------------
  ...
  print *,'PDCOMP:  ni, nj, nz', ni, nj, nk

  allocate( buoy(0:ni+1,0:nj+1,0:nk+1) )

  do k=1,nk
  do j=1,nj
  do i=1,ni
    bb(i,j,k)   = pb(i,j,k)   ! store this off for later
    buoy(i,j,k) = pb(i,j,k)   ! store this off for later
  enddo
  enddo
  enddo
...
! Mapping factors for stretched grid
  ...
  do k=1,nk
    thr0(k) = th0(k)*(1.0 + reps*qv0(k)) / (1.0 + qv0(k))  ! Theta_rho
  enddo
  thr0(0)    = thr0(1)
  thr0(nk+1) = thr0(nk)
...
!----- coefficients -----
  ...
  do k=1,nk
    cfa(k) = mh(k) * mf(k  ) * rf0(k  ) * 0.5 * (thr0(k-1)+thr0(k  ))  / (rho0(k) * thr0(k))
    cfc(k) = mh(k) * mf(k+1) * rf0(k+1) * 0.5 * (thr0(k  )+thr0(k+1))  / (rho0(k) * thr0(k))
  enddo
...
!  forcing for buoyancy pressure
  ...
  IF ( nloop .eq. -1 ) THEN
    ...
    do j=1,nj
    do i=1,ni
      deft(i,j,0)    = -betaP(i,j,1)  
      deft(i,j,nk+1) = -betaP(i,j,nk)  
    enddo
    enddo

  ELSEIF( nloop .eq. 0 ) THEN

    do k=1,nk
    do j=1,nj
    do i=1,ni
      deft(i,j,k) = betaT(i,j,k)  
    enddo
    enddo
    enddo
    ...
    do j=1,nj
    do i=1,ni
      deft(i,j,0)    = -betaT(i,j,1)  
      deft(i,j,nk+1) = -betaT(i,j,nk)  
    enddo
    enddo
  ...
  ELSEIF(nloop.eq.1)THEN
    ...
    do j=1,nj
    do i=1,ni
      wten(i,j,k) = 0.5*(buoy(i,j,k-1)+buoy(i,j,k))
    enddo
    enddo
    enddo
    ...
    do j=1,nj
    do i=1,ni
      deft(i,j,k) = mh(k) * (rf0(k+1)*wten(i,j,k+1)-rf0(k)*wten(i,j,k)) / (cp*rho0(k)*thr0(k))
    enddo
    enddo
    enddo
...
!     from total dynamic pressure)
  ...
  ELSEIF(nloop.eq.3)THEN
    ...
    write(*,*) ' Computed Divergence term'
...
! JT:  in the following, compute the dynamic pressure forcing, and buoyancy pressure forcing, for comparison with WK90, etc.
    ...
    do j=2,nj-1
    do i=2,ni-1
!!      fptdn(i,j,k)=-cp*thr0(k)*(ptdn(i,j,k+1)-ptdn(i,j,k-1))/     &
!!                   (zh(k+1)-zh(k-1))
!      fptdn(i,j,k)=fptdn(i,j,k)-cp*thr0(k)*(pi0(k+1)-pi0(k-1))/   &
!                   (zh(k+1)-zh(k-1))
!!      fpdn(i,j,k)=-cp*thr0(k)*(pdn(i,j,k+1)-pdn(i,j,k-1))/        &
!!                   (zh(k+1)-zh(k-1))
!!      fpdl(i,j,k)=-cp*thr0(k)*(pdl(i,j,k+1)-pdl(i,j,k-1))/        &
!!                   (zh(k+1)-zh(k-1))
!      fpdn(i,j,k)=fpdn(i,j,k)-cp*thr0(k)*(pi0(k+1)-pi0(k-1))/     &
!                   (zh(k+1)-zh(k-1))
!!      fpb(i,j,k)=-cp*thr0(k)*(pb(i,j,k+1)-pb(i,j,k-1))/           &
!!                   (zh(k+1)-zh(k-1)) + bb(i,j,k)
!      fpex(i,j,k)=-cp*thr0(k)*(pex(i,j,k+1)-pex(i,j,k-1))/        &
!                   (zh(k+1)-zh(k-1))
!      fpsh(i,j,k)=-cp*thr0(k)*(psh(i,j,k+1)-psh(i,j,k-1))/        &
!                   (zh(k+1)-zh(k-1))
!      fpsum(i,j,k)=fpex(i,j,k)+fpsh(i,j,k)

      fpb(i,j,k)    = -cp*thr0(k)*(pb(i,j,k+1)-pb(i,j,k-1))   / (zh(k+1)-zh(k-1))  ! pert vert BPGF
      fpbz(i,j,k)   = -cp*thr0(k)*(pb(i,j,k+1)-pb(i,j,k-1))   / (zh(k+1)-zh(k-1)) + buoy(i,j,k)
      fpbx(i,j,k)   = betaT(i,j,k) + betaP(i,j,k)
      fpby(i,j,k)   = -cp*thr0(k)*(pb(i,j+1,k)-pb(i,j-1,k))   / (2.*dy) 

      fpdnx(i,j,k)  = -cp*thr0(k)*(pdn(i+1,j,k)-pdn(i-1,j,k)) / (2.*dx)
      fpdny(i,j,k)  = -cp*thr0(k)*(pdn(i,j+1,k)-pdn(i,j-1,k)) / (2.*dy)
      fpdnz(i,j,k)  = -cp*thr0(k)*(pdn(i,j,k+1)-pdn(i,j,k-1)) / (zh(k+1)-zh(k-1))

      fpdlx(i,j,k)  = -cp*thr0(k)*(pdl(i+1,j,k)-pdl(i-1,j,k))/(2.*dx)
      fpdly(i,j,k)  = -cp*thr0(k)*(pdl(i,j+1,k)-pdl(i,j-1,k))/(2.*dy)
      fpdlz(i,j,k)  = - cp*thr0(k)*(pdl(i,j,k+1)-pdl(i,j,k-1))/(zh(k+1)-zh(k-1))

      fptdnx(i,j,k) = -cp*thr0(k)*(ptdn(i+1,j,k)-ptdn(i-1,j,k))/(2.*dx)
      fptdny(i,j,k) = -cp*thr0(k)*(ptdn(i,j+1,k)-ptdn(i,j,k-1))/(2.*dy)
      fptdnz(i,j,k) = -cp*thr0(k)*(ptdn(i,j,k+1)-ptdn(i,j,k-1))/(zh(k+1)-zh(k-1))
    enddo
    enddo
    enddo
...
! 
  ...
  subroutine pdcomp2(ni,nj,nk,wbc,ebc,sbc,nbc,dx,dy,zh,rhs3d,sol3d)
  ...
  integer, intent(in)                       :: wbc,ebc,sbc,nbc
  real,    intent(in)                       :: dx,dy
  real,    intent(in),  dimension(nk)       :: zh
...
!  forcing for buoyancy pressure
    ...
    do j=1,nj
    do i=1,ni
      deft(i,j,0)    = rhs3d(i,j,1)  
      deft(i,j,nk+1) = rhs3d(i,j,nk)  
    enddo
    enddo
!-----------------------------------------------------------------------
!  p solver
