...
!       pb(i,j,k)  = g*((thv(i,j,k)-thv0(k))/thv0(k))
    ...
    DO i=2,nx-1
      ...
      beta(i,j,k) = (den(i-1,j,k) - 2.0*den(i,j,k) + den(i+1,j,k)) / (dx**2) &
                  + (den(i,j-1,k) - 2.0*den(i,j,k) + den(i,j+1,k)) / (dy**2)

    ENDDO
    ENDDO

    DO j = 2,ny-1
      beta(1,j,k)  = (den(nx,  j,  k) - 2.0*den(1, j,k) + den(2, j,  k)) / (dx**2) &
                   + (den(1,   j-1,k) - 2.0*den(1, j,k) + den(1, j+1,k)) / (dy**2)
      beta(nx,j,k) = (den(nx-1,j,  k) - 2.0*den(nx,j,k) + den(1, j,  k)) / (dx**2) &
                   + (den(nx,  j-1,k) - 2.0*den(nx,j,k) + den(nx,j+1,k)) / (dy**2)
    ENDDO
    DO i = 2,nx-1
      beta(i,1,k)  = (den(i-1,   1,k) - 2.0*den(i, 1,k) + den(i+1, 1,k)) / (dx**2) &
                   + (den(i,  ny-1,k) - 2.0*den(i, 1,k) + den(i,   2,k)) / (dy**2)
      beta(i,ny,k) = (den(i-1,ny,  k) - 2.0*den(i,ny,k) + den(i+1,ny,k)) / (dx**2) &
                   + (den(i,  ny-1,k) - 2.0*den(i,ny,k) + den(i,   1,k)) / (dy**2)
    ENDDO
...
! Write data to netCDF-format file:
    ...
    call writenc(outfile,nx,ny,nz, bb, pb, fpb, pdl, pdn, fptdnx,  &
...
!=================== Write netCDF ==========================
    subroutine writenc(filename, nx, ny, nz,                                 &
               bb, pb, bdpdz, pdl, pdn, fptdnx, fptdny, fptdnz, fpbx, fpby, fpbz, &
    ...
    real, dimension(nx,ny,nz), intent(in) :: bdpdz, bb, pb, pdl, pdn
    ...
    integer :: bdpdzVarID, bbVarID, pbVarID, pdlVarID, pdnVarID
...
!------------ Define dimensions and variables -------------
    ...
    status = nf90_def_var(ncid,"buoy",nf90_float, &
                         (/niDimID,njDimID,nkDimID,timeDimID/),bbVarID)
    if(status /= nf90_NoErr) write(*,*) nf90_strerror(status)
...
!------------ Write dimensions and variables -------------
    ...
    status = nf90_put_var(ncid,bbVarID,bb)
    if(status /= nf90_NoErr) write(*,*) nf90_strerror(status)
